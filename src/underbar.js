(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    if(arguments.length > 1){
      return _.reduce(collection, function(truth, item){
        if (!iterator(item)) truth = false; 
        return truth;
      }, true)
    }else{
      return _.reduce(collection, function(truth, item){
        if (item !== truth) truth = false; 
        return truth;
      }, true)
    }
  };
  ...
  _.some = function(collection, iterator) {
   ...
   if(arguments.length > 1){
      return _.reduce(collection, function(truth, item){
        if (iterator(item)) truth = true; 
        return truth;
      }, false)
    }else{
      return _.reduce(collection, function(truth, item){
        if (item == true) truth = true; 
        return truth;
      }, false)
    }
  };
  ...
  _.memoize = function(func) {
    ...
    return function(){
      ...
      for(var i = 0; i< args.length; i++){
        if(JSON.stringify(args)==JSON.stringify(arrayList[i])) alreadyCalled = true;
        if( !alreadyCalled ){
  ...
  _.shuffle = function(array) {
    ...
    while(copy.length > 0){
      var k = Math.floor(Math.random()*(copy.length-0));
      final.push(copy[k]);
      copy.splice(k,1);
    }

    console.log('original', array, 'copy', copy,'final', final);
    return final;
